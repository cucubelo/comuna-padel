...
// Función auxiliar para detectar errores de autenticación
...
export async function createAccessRequest({
  ...
  try {
    ...
    if (isAuthError(error)) {
      console.warn("Error de autenticación detectado en createAccessRequest");
      return {
        data: null,
...
export async function getGroupAccessRequests(groupId: string) {
  try {
    ...
    if (isAuthError(error)) {
      console.warn(
        "Error de autenticación detectado en getGroupAccessRequests"
      );
      return {
        data: null,
        error: "Tu sesión ha expirado. Por favor, inicia sesión nuevamente.",
...
export async function getUserAccessRequests(userId: string) {
  try {
    ...
    // Manejo específico de errores de autenticación
    if (isAuthError(error)) {
      console.warn("Error de autenticación detectado en getUserAccessRequests");
      return {
        data: null,
        error: "Tu sesión ha expirado. Por favor, inicia sesión nuevamente.",
      };
...
export async function approveAccessRequest(
  ...
  try {
    ...
    // Manejo específico de errores de autenticación
    if (isAuthError(error)) {
      console.warn("Error de autenticación detectado en approveAccessRequest");
      return {
        error: "Tu sesión ha expirado. Por favor, inicia sesión nuevamente.",
      };
    }
...
export async function rejectAccessRequest(
  ...
  try {
    ...
    // Manejo específico de errores de autenticación
    if (isAuthError(error)) {
      console.warn("Error de autenticación detectado en rejectAccessRequest");
      return {
        error: "Tu sesión ha expirado. Por favor, inicia sesión nuevamente.",
      };
    }
...
export async function hasUserPendingRequest(
  groupId: string,
  userId: string
): Promise<boolean> {
  try {
    ...
    // Para esta función, simplemente retornamos false en caso de error de auth
    // ya que es una función de verificación y no crítica
    if (isAuthError(error)) {
      console.warn("Error de autenticación detectado en hasUserPendingRequest");
    }
