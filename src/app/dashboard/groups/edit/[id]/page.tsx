...
export default function EditGroupPage() {
  ...
  const fetchGroup = useCallback(async () => {
    try {
      // First check if user is admin of this group
      ...
      // Si el grupo no tiene país configurado, usar el del perfil del usuario
      let countryName = groupData.country || "";
      let countryCode = groupData.country_code || "";

      if (!countryCode && profile?.country_code) {
        countryCode = profile.country_code;

        // Si tenemos country_code pero no country, buscar el nombre del país
        if (!countryName && profile.country_code) {
          try {
            const countryData = await getCountryByCode(profile.country_code);
            countryName = countryData?.country_name || profile.country || "";
          } catch (error) {
      ...
      setFormData({
        ...
        place_name: groupData.place_name || "",
        group_type: groupData.group_type || "private",
      });

      // Si el grupo ya tiene datos de ubicación válidos, marcar como confirmado
      // Para grupos existentes sin postal_code, solo requerir city
      if ((groupData.postal_code && groupData.city) || (!groupData.postal_code && groupData.city)) {
        setIsLocationConfirmed(!!groupData.city);
      }
    } catch (error) {
  ...
  }, [groupId, user?.id, router, profile?.country_code, profile?.country]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!isLocationConfirmed) {
      newErrors.postal_code =
        "Debes seleccionar un código postal válido de la lista";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        submit: "Error al actualizar el grupo. Inténtalo de nuevo.",
      });
    } finally {
        ...
        <div className="min-h-screen bg-bg-secondary flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-text-secondary mb-4">
              El grupo que buscas no existe o no tienes permisos para editarlo.
            </p>
            <button
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-text-secondary font-open-sans mt-2">
                  Modifica la información de tu grupo: {group.name}
                </p>
              </div>
            </div>
          </div>
