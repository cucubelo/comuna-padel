...
export default function ProfileCompletionBanner({ className = '' }: ProfileCompletionBannerProps) {
  ...
  const isProfileIncomplete = () => {
    ...
    const hasLocation = profile.place_name || profile.city || profile.location
    
    const requiredFields = [
      profile.phone,
      profile.skill_level && profile.skill_level > 1, // skill_level debe ser mayor a 1 (no beginner)
      ...
      <div className="flex items-start space-x-3 sm:space-x-0">
        <div className="flex-shrink-0">
          <div className="bg-accent-primary/10 p-2 sm:p-3 rounded-xl">
            <svg className="h-5 w-5 sm:h-6 sm:w-6 text-accent-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
        ...
        <div className="sm:ml-5 flex-1 min-w-0">
          ...
          <div className="bg-bg-main/50 rounded-lg p-3 sm:p-4 mb-4 sm:mb-5 border border-accent-primary/10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs sm:text-sm font-semibold text-text-main font-open-sans">Información pendiente:</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {missingFields.map((field, index) => (
                <div key={index} className="flex items-center gap-2 text-xs sm:text-sm text-text-secondary font-open-sans">
